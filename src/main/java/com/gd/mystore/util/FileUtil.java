...
public class FileUtil {
	...
	public Map<String, String> fileupload(MultipartFile uploadFile, String folderName) {
		...
		if(!filePathDir.exists()) {	// 해당 경로의 폴더가 존재하지 않을경우
			filePathDir.mkdirs();	// 해당 폴더 만들기
		}
		
		// 2) 파일명 수정 작업
		String originalFilename = uploadFile.getOriginalFilename(); // "xxxx.jpg" | "xxx.tar.gz"
		...
		// 3) 업로드 (폴더에 파일 저장)
		try {
			uploadFile.transferTo(new File(filePathDir, filesystemName));	// 어떤 폴더에 어떤 이름으로 저장
		...
		// db에 기록할 데이터 다시 반환
		Map<String, String> map = new HashMap<>();	// Map에 담는 이유: 범용성 있게 사용하기 위해
		map.put("filePath", filePath);
		map.put("originalName", originalFilename);
		map.put("filesystemName", filesystemName);
//		return filePath, originalFilename, filesystimName;
		return map;
	}
}